...
function getMostHealing(id, output, client){
    ...
    var queryString = "SELECT total_heal, champion_name, champion_key FROM game INNER JOIN champion USING (champion_id) WHERE summoner_id = " + id + ' ORDER BY total_heal DESC LIMIT 1';
...
function getMostMinionKilled(id, output, client){
    ...
    var queryString = "SELECT minion_kills, champion_name, champion_key FROM game INNER JOIN champion USING (champion_id) WHERE summoner_id = " + id + ' ORDER BY minion_kills DESC LIMIT 1';
