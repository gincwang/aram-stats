...
//createTable(client);     //Comment out this line if table already exists
...
function createTable(client){
    client.query('CREATE TABLE champion(champion_id SMALLINT primary key, champion_title VARCHAR(50) not null, champion_name VARCHAR(50) not null, champion_key VARCHAR(50) not null)', function(err, result){
        if(err){
            client.end;
        }
    });
    ...
    riotSeeder.getChampions()
        .then(function(result){
            ...
            for(var champKey in result.data){
                ...
                queryString = "INSERT INTO champion (champion_id, title, name, key) VALUES (" + champion_id + ", $$" + title + "$$, $$" + name + "$$, $$" + key + "$$) ON CONFLICT DO NOTHING";
